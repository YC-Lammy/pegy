...
impl Parser{
    pub fn parse_alternative(&mut self, tokens: &[TokenTree], pos: &mut usize, is_silent: bool) -> syn::Result<TokenStream>{
        if *pos == tokens.len(){
    ...
    pub fn parse_terms(&mut self, tokens: &[TokenTree], pos: &mut usize, is_silent: bool) -> syn::Result<TokenStream>{
        let mut terms = TokenStream::new();

        let mut breaking = "'t".to_string();
        breaking.push_str(&std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos().to_string());
    ...
    pub fn parse_term(&mut self, tokens: &[TokenTree], pos: &mut usize, is_silent: bool) -> syn::Result<TokenStream>{
        ...
        match n{
            ...
            TokenTree::Ident(i) => {
                ...
                stream = quote::quote_spanned!(i.span() => <#id as ::pegy::Parse>::parse(src).await);
            }
                ...
                if p.as_char() == '$'{
                    if let Some(TokenTree::Ident(id)) = tokens.get(*pos){
                        *pos += 1;
                        ...
                        return Err(syn::Error::new(p.span(), "expected ident behind binding declaration"));
                    }
                } else if p.as_char() == '!'{
                    let term = self.parse_term(tokens, pos, is_silent)?;
    ...
    pub fn parse_quantifier(&mut self, tokens: &[TokenTree], pos: &mut usize, is_silent: bool, term: TokenStream) -> syn::Result<TokenStream>{
        if let Some(TokenTree::Punct(p)) = tokens.get(*pos){
            if p.as_char() == '+'{
                *pos += 1;
                if is_silent{
    ...
    pub fn parse_character_class(&mut self, stream: TokenStream) -> syn::Result<TokenStream>{
        ...
        for (start, end) in ranges{
            if stream.is_empty(){
                stream.extend(
                    quote::quote_spanned!{ start.span() =>
