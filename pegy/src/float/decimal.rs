...
#[inline]
pub async fn parse_decimal<S:Source>(s: &mut S) -> Decimal {
    // can't fail since it follows a call to parse_number
    let mut d = Decimal::default();

    if s.match_char('-').await{
        d.negative = true;

    } else if s.match_char('+').await{
    };

    // skip '0'
    while s.match_char('0').await{};

    while let Some(ch) = s.match_char_range('0'..='9').await{
        d.try_add_digit(ch as u8 - '0' as u8);
    }
    ...
    if s.match_char('e').await || s.match_char('E').await {
        ...
        while let Some(ch) = s.match_char_range('0'..='9').await{
            if exp_num < 0x10000 {
                exp_num = 10 * exp_num + (ch as i32 - '0' as i32);
            }
        }
