...
const CONT_MASK: u8 = 0x3F;
    ...
    #[inline]
    pub unsafe fn next_code_point<'a, I: Iterator<Item = &'a u8>>(bytes: &mut I) -> Option<(u32, usize)> {
        let mut i = 1;
...
#[cfg(feature = "futures")]
pub struct AsyncStrBufReader<T:futures::AsyncRead + Unpin>{
    reader: T,
    pub buffer: Vec<u8>,
    pos: usize,
    is_eof: bool,
...
#[cfg(feature = "futures")]
impl<T:futures::AsyncRead + Unpin> AsyncStrBufReader<T>{

    pub const fn new(reader: T) -> Self{
        Self { reader, buffer: Vec::new(), pos: 0, is_eof: false }
    }
    ...
    async fn read_buf(&mut self) -> Option<usize>{
        ...
        if !self.is_eof{
            ...
            unsafe{
                ...
                let buf = core::slice::from_raw_parts_mut(ptr, 128);

                match self.reader.read(buf).await{
                    Ok(l) => {
                        self.buffer.set_len(self.buffer.len() + l);
...
#[cfg(feature = "futures")]
impl<T:futures::AsyncRead + Unpin> Source for AsyncStrBufReader<T>{
    fn current_position(&self) -> usize {
        self.pos
    }
    fn set_position(&mut self, pos: usize) {
    ...
    async fn peek(&mut self) -> Option<Character> {
        ...
        if let Some((c, l)) = self.next_code_point().await{
            return Some(Character { ch: unsafe{char::from_u32_unchecked(c)}, length: l })
        } else{
            return None
        }
    }
